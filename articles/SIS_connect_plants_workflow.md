...
document. Bachman *et al*. (2024) predicted extinction risk for all
species of flowering plants (Angiosperms). You can filter on species
that are confidently predicted to be LC, or manually generate a data
frame from a list as shown below.
...
Red Lists. You can also run the built-in LC test function
...
This will check the names against the GBIF names backbone as well as the
World Checklist of Vascular Plants using the `rWCVP` package. The
...
i.e. where a name was not at species level, or either of the name
sources (GBIF, WCVP) does not treat the name as accepted. In some cases
you may want to override the results, for example if GBIF treats a name
as a synonym, but WCVP treats it as accepted you can set the parameter
`override_gbif_status = TRUE` and `clean_keys` will treat the GBIF name
as accepted at species level. Similarly, set
`override_wcvp_status = TRUE` if you want to do the same for WCVP.
However, note that using a WCVP name that is not accepted will cause
problems with the workflow downstream e.g. native ranges and other
information from WCVP are only available for accepted species.
